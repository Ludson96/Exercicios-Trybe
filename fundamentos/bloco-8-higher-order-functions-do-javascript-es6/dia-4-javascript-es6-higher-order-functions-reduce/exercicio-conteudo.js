...
// const testeReduce = numbers.reduce((acc, number) => number % 2 === 0 ? acc + number : acc );
...
const melhorNota = estudantes.map((element) => {
  const relacao = {
    name: element.nome,
    materia: element.materias.reduce((acc, curr) => acc.nota > curr.nota ? acc : curr).name,
  };
  return relacao;
})
